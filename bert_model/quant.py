...
while batch_size <= max_batch_size:
    try:
        ...
        model.eval()  # Switch to evaluation mode before quantization
        ...
        for batch in test_dataloader:
            batch = {k: v.to(device) for k, v in batch.items()}
            with torch.no_grad():
                outputs = quantized_model(input_ids=batch['input_ids'], attention_mask=batch['attention_mask'], labels=batch['label'])
        ...
        tokenizer.save_pretrained('trained_model_quantized')
